...
export const announcementsRoutes = (cfg: APIConfig, store: Store, apsystem: ActivityPubSystem) => async (server: FastifyTypebox): Promise<void> => {
  server.get<{
    ...
    return await reply.headers({ 'Content-Type': 'application/ld+json' }).send(actor)
  })
  ...
  server.get<{
    ...
    return await reply.headers({ 'Content-Type': 'application/ld+json' }).send(await apsystem.announcements.getOutbox())
  })
  ...
  server.get<{
    ...
    return await reply.headers({ 'Content-Type': 'application/ld+json' }).send(activity)
  })
}
